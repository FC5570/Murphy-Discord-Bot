  ...
  constructor(options) {
      ...
      (message, command, type, missing) => {
        if (type.toLowerCase() === "client") {
          const embed = new MessageEmbed()
            ...
            .setDescription(
              `**I am** missing **${firstUpperCase(
                missing.join(", ").replace(/_/gi, " ").toLowerCase()
              )}** permissions, please give me the required permissions for the \`${command}\` command to work.`
            )
            .setColor("RED")
            .setTimestamp()
            .setAuthor(
          ...
          const embed = new MessageEmbed()
            .setTitle("Missing Permissions")
            .setDescription(
              `**You are** missing **${firstUpperCase(
                missing.join(", ").replace(/_/gi, " ").toLowerCase()
              )}** permissions, please make sure you have the required permissions for the \`${command}\` command to work.`
            )
            .setColor("RED")
            .setTimestamp()
            .setAuthor(
    ...
    this.commandHandler.on("error", async (error, message, command) => {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `An error occured, please DM **${await (
            await this.users.fetch(this.ownerID)
          ).tag}** if this keeps happening.\n\nError: **${error.stack}**`
        )
        .setAuthor(
          message.author.username,
          message.author.displayAvatarURL({ dynamic: true })
      ...
      const embed2 = new MessageEmbed()
        ...
        .setDescription(
          `**${message.author.tag} (${message.author.id})** ran a command which errored\nCommand: **${command}\n**Server: **${message.guild.name} (${message.guild.id})**\nChannel: **${message.channel.name} (${message.channel.id})**\n\nError: **${error.stack}**`
        )
        .setTimestamp()
        .setColor("RED")
        .setAuthor(
