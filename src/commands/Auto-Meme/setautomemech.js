...
class SetAutoMemeCh extends Command {
    constructor() {
        super('setautomemech', {
            ...
            description: {
                content: 'Sets an automemechannel for the server where memes will be sent every x number of seconds/minutes/hours.',
                usage: 'setautomemech <channel> <time>',
                example: [
                    "setautomemech #auto-memes 5m",
                    "setautomemech #memes 15s"
                ]
            },
            clientPermissions: ['VIEW_CHANNELS', 'READ_MESSAGE_HISTORY', 'EMBED_LINKS'],
    ...
    async exec(message, { channel, time }) {
        ...
        await this.client.db.set(`automeme-${message.guild.id}`, [ channel.id, time]) 
        message.channel.send(`Successfully set <#${channel.id}> as the automeme channel.`)
        setInterval(async () => {
